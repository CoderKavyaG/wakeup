...
export async function POST(request: Request) {
  try {
    ...
    if (!session?.user?.id) {
      console.warn("[API Machine Launch] Unauthorized request to", request.url);
      return NextResponse.json({ error: "unauthorized" }, { status: 401 });
    }

    const body = await request.json();
    console.log("[API Machine Launch] Request body:", body);

    const res = await agentFetch("/launch", {
      method: "POST",
      body,
    });

    console.log("[API Machine Launch] agentFetch response status:", res.status);
